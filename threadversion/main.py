...
def thread_subscribe(mqtt_subscribe, topic, producer_kafka,thread_sub):
    ...
    threading.Thread()
    thread_sub = threading.Thread(target=mqtt_subscribe.processing_data, args=[producer_kafka, 'key'])
    thread_sub.start()
...
if __name__ == '__main__':
    ...
    thread1 = threading.Thread(target=thread_subscribe,
                               args=[mqtt_subscribe1, 'elevator/#', producer_kafka1, thread_sub1])
    thread1.start()

    thread_sub2 = None
    thread2 = threading.Thread(target=thread_subscribe,
                               args=[mqtt_subscribe2, 'elevator/#', producer_kafka2, thread_sub2])
    thread2.start()

    thread_sub3 = None
    thread3 = threading.Thread(target=thread_subscribe,
                               args=[mqtt_subscribe3, 'elevator/#', producer_kafka3, thread_sub3])
    thread3.start()

    thread_sub4 = None
    thread4 = threading.Thread(target=thread_subscribe,
                               args=[mqtt_subscribe4, 'elevator/#', producer_kafka4, thread_sub4])
    thread4.start()
